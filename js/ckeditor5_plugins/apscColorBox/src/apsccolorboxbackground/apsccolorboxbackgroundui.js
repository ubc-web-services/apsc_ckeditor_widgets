...
export default class ApscColorBoxBackgroundUI extends Plugin {

  /**
   * @inheritDoc
   */
  static get pluginName() {
    return 'ApscColorBoxBackgroundUI';
  }
  ...
  init() {
    ...
    const command = editor.commands.get('apscColorBoxBackground');

    editor.ui.componentFactory.add('apscColorBoxBackgroundOptions', locale => {
      ...
      dropdownView.buttonView.set({
        ...
        tooltip: true,
        withText: true,
        class: 'cke5-apsccolorbox-bgcolor-insert-button',
      });
...
function _prepareListOptions(options, command) {
  ...
  const activecommand = 'apscColorBoxBackground';
  let alloptions = backgroundStyles.map(style => {
    return style;
  });
